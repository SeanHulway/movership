class EstimatesController < ApplicationController
  ...
  def index
    @estimate = Estimate.all.order("created_at DESC")
  end

  def show
    sec = (calculate_total_cube(@estimate)*7/1000.00 * 3600).to_i
    min, sec = sec.divmod(60)
    hour, min = min.divmod(60)
    @estimate_weight = calculate_total_cube(@estimate)*7
  ...
  def create
    ...
    if @estimate.save
      redirect_to @estimate, notice: "Estimate Sent"
    else
      render 'new'
    end
  end
  ...
  def destroy
    @estimate.destroy
    redirect_to estimate_path, notice: "Successfully deleted estimate"
  end
  ...
  def calculate_total_cube(estimate)
    total = 0;
    estimate.rooms.each do |room|
